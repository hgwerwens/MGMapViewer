...
public class SplineProfileTestContextMTB implements IfSplineProfileContextMTB {
    ...
    public SplineProfileTestContextMTB(int power, int sUp, int sDn, boolean checkAll) {
        this(power, sUp, sDn, checkAll, true);
    }

    public SplineProfileTestContextMTB(int power, int sUp, int sDn) {
        this(power, sUp, sDn, true);
    }

    public SplineProfileTestContextMTB(int sUp, int sDn, boolean checkAll) {
    ...
    private static class TestContextMTBInternal implements IfSplineProfileContextMTB {
        ...
        private TestContextMTBInternal(int power, int sUp, int sDn, boolean checkAll, boolean withRef) {
            this.power = power;
            this.sUp = sUp;
            this.sDn = sDn;
            ...
            indRefDnSlope = 3;
//        slopesAll[indRefDnSlope]=slopesAll[indRefDnSlope] -  0.04f + 0.02f*sDn/100f;
            float refslope = slopesAll[indRefDnSlope];
            float dlstretch = 0.2f + (0.35f * sDn / 100f);
            ...
            float facDn = 13f - 2f*sDn / 100f;

            for (sc = 0; sc < sdistFactforCostFunct.length; sc++) {
                for (int i = 0; i < slopesAll.length; i++) {
                    if (slopesAll[i] < 0)
                        distFactforCostFunct[sc][i] = sdistFactforCostFunct[sc]; //(1f + (sdistFactforCostFunct[sc] - 1f) * 0.9f); //
